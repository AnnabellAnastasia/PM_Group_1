...
const controller = {
  ...
  join: async (req: any, res: any, next: any) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "User successfully joined the group" });
    } catch (err) {
  ...
  leave: async (req: any, res: any, next: any) => {
    try {
      ...
      if (groupIndexInUser > -1) {
        ...
        return res
          .status(400)
          .json({ error: "Group is not in the user's joined groups" });
      }
      await group.save();
      await user.save();
      return res
        .status(200)
        .json({ message: "User successfully left the group" });
    } catch (err) {
  ...
  delete: async (req: any, res: any, next: any) => {
    try {
      const {id, usr} = req.body;
        ...
        groupPosts.deleteMany({post: {$in: group.posts}})
        group.deleteOne(id);
        return res.status(200).send("group deleted");
    } catch(err) {
        console.error(err, "there was an error deleting");
    }
